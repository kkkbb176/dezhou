 ...
 * 被再加注后的 Hero 最佳应对       （FOLD / CALL / 5Bet / 全下，零点统一）
...
export type PreflopRaiseSizeFacts = {
  ...
  readonly reraiseBranchKind: 'FOLD' | 'CALL' | 'RERAISE' | 'ALL_IN' | 'FOLD_ONLY_UNAVAILABLE';
  /** Hero 面对 4Bet 的 FOLD / CALL / 5Bet 应对；存在合法 5Bet 时为 true */
  readonly heroFiveBetExpanded: boolean;
  /** 合法 5Bet 候选（按金额升序；没有合法候选时为空） */
  readonly heroFiveBetCandidates: readonly {
    readonly labelZh: string;
    readonly sizeChips: number;
    readonly isAllIn: boolean;
    readonly minLegal: boolean;
    readonly branchEV: number;
    readonly foldLikelihood: number;
    readonly callLikelihood: number;
    readonly reRaiseLikelihood: number;
    readonly heroEquityVs5BetCallRange: PreflopEquityFact;
    readonly finalPot: number;
    readonly heroAdditional: number;
    readonly uncalledReturn: number;
    readonly supported: boolean;
    readonly unsupportedZh: string | null;
  }[];
  /** 被 4Bet 后，Hero 实际选择的后续应对 */
  readonly heroFiveBetChoice: 'FOLD' | 'CALL' | 'RERAISE' | 'ALL_IN' | 'FOLD_ONLY_UNAVAILABLE';
  readonly heroFiveBetBranchEV: number | null;
  readonly heroFiveBetUnsupportedZh: string | null;
...
export function buildPreflopRaiseFacts(input: BuildPreflopRaiseFactsInput): PreflopRaiseFactsBuild {
  ...
  const facts: PreflopRaiseFacts = Object.freeze({
    ...
    noteZh:
      ...
      `零点 = 弃牌 ≡ 0，单位 = 筹码｜被再加注分支展开 Hero 的 FOLD / CALL / 5Bet / 全下（有合法候选时）`,
  });
...
const ASSUMPTIONS_ZH: readonly string[] = Object.freeze([
  ...
  '被再加注分支展开 Hero 的 FOLD / CALL / 5Bet / 全下；5Bet 的对手响应继续复用同一响应模型',
...
function buildOneSize(args: {
  ...
  /*
   * ============================================================
   * Hero 面对 4Bet：FOLD / CALL / 5Bet / 全下
   * ============================================================
   *
   * 4Bet 不是终点。Hero 若仍有筹码和加注权，必须在**同一个原始
   * 3Bet 决策节点**上比较 5Bet 分支；否则常规 3Bet 会系统性低估
   * 「打到 4Bet 后再全下」的价值，只剩 FOLD / CALL 两选一。
   *
   * 本段复用同一个 buildPreflopRaiseResponse 和 raiseEVOf：
   * - Hero 的 5Bet 到达范围 = 对手 4Bet 条件范围（reRaiseEntries）
   * - 对手面对 5Bet 的响应 = 同一响应模型按新价格/新范围条件化
   * - 5Bet 被跟注时的权益 = 对手跟注桶条件范围
   */
  const heroFiveBetCandidates: PreflopRaiseSizeFacts['heroFiveBetCandidates'] = (() => {
    if (reRaisePlan === null || eqVsReraise.value === null) return Object.freeze([]);
    const heroMaxTo = args.legal.allInToAmount;
    const rawMinFiveBetTo = reRaisePlan.reRaiseTo + (reRaisePlan.reRaiseTo - raiseTo);
    const minFiveBetTo = Math.min(rawMinFiveBetTo, heroMaxTo);
    if (!(minFiveBetTo > reRaisePlan.reRaiseTo + 1e-9)) return Object.freeze([]);

    const afterHeroRaise = applyAction(state, {
      playerId: hero.id,
      type: ActionType.RAISE,
      amount: raiseTo,
    });
    if (!afterHeroRaise.ok) return Object.freeze([]);
    const afterVillainReRaise = applyAction(afterHeroRaise.state, {
      playerId: opponent.id,
      type: ActionType.RAISE,
      amount: reRaisePlan.reRaiseTo,
    });
    if (!afterVillainReRaise.ok) return Object.freeze([]);
    const villainAfterReRaise = afterVillainReRaise.state.players.find((p) => p.id === opponent.id);
    if (villainAfterReRaise === undefined) return Object.freeze([]);
    const potAfterVillainReRaise = computePot(afterVillainReRaise.state);
    const heroInitialRaiseAdd = Math.max(0, raiseTo - args.heroStreetCommitted);
    const foldWinFromOriginalNode = potAfterVillainReRaise - heroInitialRaiseAdd;

    const candidateTos = new Set<number>();
    candidateTos.add(minFiveBetTo);
    candidateTos.add(heroMaxTo);
    const ordered = [...candidateTos]
      .filter((to) => to > reRaisePlan.reRaiseTo + 1e-9 && to <= heroMaxTo + 1e-9)
      .sort((a, b) => a - b);

    return Object.freeze(ordered.map((heroFiveBetTo) => {
      const base = Object.freeze({
        labelZh: heroFiveBetTo >= heroMaxTo - 1e-9
          ? '全下 5Bet'
          : heroFiveBetTo <= minFiveBetTo + 1e-9
            ? '最小 5Bet'
            : '5Bet',
        sizeChips: heroFiveBetTo,
        isAllIn: heroFiveBetTo >= heroMaxTo - 1e-9,
        minLegal: heroFiveBetTo <= minFiveBetTo + 1e-9,
      });

      const heroAdditional = Math.max(0, heroFiveBetTo - args.heroStreetCommitted);
      const heroFiveBetAddAtFourBetNode = Math.max(0, heroFiveBetTo - raiseTo);
      const villainStreetBeforeFiveBet = reRaisePlan.reRaiseTo;
      const villainAddRaw = Math.max(0, heroFiveBetTo - villainStreetBeforeFiveBet);
      const villainRemainingAfterReRaise = Math.max(0, villainAfterReRaise.remainingStack);
      const villainAdd = Math.min(villainAddRaw, villainRemainingAfterReRaise);
      const villainIsAllInByCall = villainAdd >= villainRemainingAfterReRaise - 1e-9;
      const villainStreetTotalAfterCall = villainStreetBeforeFiveBet + villainAdd;
      const heroContestedAdd = Math.min(
        heroAdditional,
        Math.max(0, villainStreetTotalAfterCall - args.heroStreetCommitted),
      );

      const afterHeroFiveBet = applyAction(afterVillainReRaise.state, {
        playerId: hero.id,
        type: ActionType.RAISE,
        amount: heroFiveBetTo,
      } as never);
      if (!afterHeroFiveBet.ok || !(villainAdd > 0)) {
        return Object.freeze({
          ...
          callLikelihood: 0,
          reRaiseLikelihood: 0,
          heroEquityVs5BetCallRange: { value: null, method: 'NOT_AVAILABLE' as const, iterations: 0, confidenceHalfWidth: null },
          finalPot: 0,
          heroAdditional,
          uncalledReturn: Math.max(0, heroAdditional - heroContestedAdd),
          supported: false,
          unsupportedZh: '5Bet 的引擎资金流不可计算',
        });
      }

      const winnableAfterVillainCall = previewCommit(afterHeroFiveBet.state, opponent.id, villainAdd).winnable;
      const response5 = buildPreflopRaiseResponse({
        arrivalEntries: response.reRaiseEntries,
        currentPot: potAfterVillainReRaise,
        heroAdd: heroFiveBetAddAtFourBetNode,
        villainAdd,
        finalPot: winnableAfterVillainCall,
        tendencies: args.tendencies,
        heroIsAllIn: heroFiveBetTo >= heroMaxTo - 1e-9,
        villainIsAllInByCall: villainIsAllInByCall,
        actionHistoryScale: { callScale: 0.92, reRaiseScale: 0.85 },
      });
      if (response5 === null) {
        return Object.freeze({
          ...base,
          branchEV: reraiseFoldBranchEV,
          foldLikelihood: 0,
          callLikelihood: 0,
          reRaiseLikelihood: 0,
          heroEquityVs5BetCallRange: { value: null, method: 'NOT_AVAILABLE' as const, iterations: 0, confidenceHalfWidth: null },
          ...
          uncalledReturn: Math.max(0, heroAdditional - heroContestedAdd),
          supported: false,
          unsupportedZh: '对手面对 5Bet 的响应模型不可计算',
        });
      }

      const equityVs5Call = args.equityOf(response5.callEntries, 3601 + heroFiveBetTo);
      if (equityVs5Call.value === null) {
        return Object.freeze({
          ...base,
          branchEV: reraiseFoldBranchEV,
          foldLikelihood: response5.foldLikelihood,
          callLikelihood: response5.callLikelihood,
          reRaiseLikelihood: response5.reRaiseLikelihood,
          heroEquityVs5BetCallRange: equityVs5Call,
          finalPot: winnableAfterVillainCall,
          heroAdditional,
          uncalledReturn: Math.max(0, heroAdditional - heroContestedAdd),
          supported: false,
          unsupportedZh: '5Bet 被跟注时的条件权益不可计算',
        });
      }

      const fiveBetEV = raiseEVOf({
        foldLikelihood: response5.foldLikelihood,
        callLikelihood: response5.callLikelihood,
        reRaiseLikelihood: response5.reRaiseLikelihood,
        currentPot: foldWinFromOriginalNode,
        heroContestedAdd,
        finalPot: winnableAfterVillainCall,
        equityVsRaiseCall: equityVs5Call.value,
        reraiseBranchEV: -heroContestedAdd,
      });
      return Object.freeze({
        ...base,
        branchEV: fiveBetEV,
  ...
  const supportedFiveBets = heroFiveBetCandidates.filter((c) => c.supported);
  const bestFiveBet = supportedFiveBets.length === 0
    ? null
    : supportedFiveBets.reduce((a, b) => (b.branchEV > a.branchEV ? b : a));
  const heroFiveBetBranchEV = bestFiveBet === null ? null : bestFiveBet.branchEV;
  const heroFiveBetUnsupportedZh =
    heroFiveBetCandidates.length > 0 && bestFiveBet === null
      ? '存在 5Bet 候选，但没有一个拥有可复算的后续响应与条件权益 ⇒ 不伪造精确 EV'
      : null;

  const reraiseBranchEV =
    reraiseCallBranchEV === null
      ? reraiseFoldBranchEV
      : Math.max(reraiseFoldBranchEV, reraiseCallBranchEV, heroFiveBetBranchEV ?? Number.NEGATIVE_INFINITY);
  const reraiseBranchKind: PreflopRaiseSizeFacts['reraiseBranchKind'] =
    heroFiveBetBranchEV !== null && heroFiveBetBranchEV > Math.max(reraiseFoldBranchEV, reraiseCallBranchEV ?? Number.NEGATIVE_INFINITY) + 1e-9
      ? (bestFiveBet?.isAllIn ? 'ALL_IN' : 'RERAISE')
      : reraiseCallBranchEV === null
        ? 'FOLD_ONLY_UNAVAILABLE'
        : reraiseCallBranchEV > reraiseFoldBranchEV + 1e-9
          ? 'CALL'
          : 'FOLD';
  const heroFiveBetChoice: PreflopRaiseSizeFacts['heroFiveBetChoice'] = reraiseBranchKind;
  ...
  return Object.freeze({
    ...
    reraiseBranchEV,
    reraiseBranchKind,
    heroFiveBetExpanded: heroFiveBetCandidates.length > 0 && bestFiveBet !== null,
    heroFiveBetCandidates,
    heroFiveBetChoice,
    heroFiveBetBranchEV,
    heroFiveBetUnsupportedZh,
