...
function qualitySummaryZh(quality: GtoQuality, evidence: GtoQualityEvidence): string {
  const targetGap = evidence.targetGap ?? 0;
  const reachedTarget =
    targetGap > 0 && evidence.brGapTotal !== null && evidence.brGapTotal < targetGap;
  const gapText =
  ...
  switch (quality) {
    case GtoQuality.LOW_CONVERGENCE:
      return reachedTarget
        ? `未做稳定性复核：${gapText}，迭代 ${evidence.iterations ?? '未知'} 次。已达到本次收敛目标，但缺少重复求解稳定性证据，因此仍降为低收敛；频率可作近似参考，不可当作均衡。`
        : `低收敛：${gapText}，迭代 ${evidence.iterations ?? '未知'} 次。频率是**未收敛快照**，不可当作均衡。`;
