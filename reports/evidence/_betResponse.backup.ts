...
export function classifyResponse(input: ResponseInput): ResponseClassification {
   ...
   */
  /*
   * 规则层只能用公共牌面相对强度：`versusHero` 是「该组合 vs Hero 真实底牌」
   * 的比较结果，只能留在范围汇总/诊断层；若混入这里，换一张 Hero 底牌就会
   * 改变同一名对手、同一公共牌、同一组合的行动规则（真实信息边界泄露）。
   *
   * 但「去 Hero 信息」不等于把档位基准也一起删掉：`tier` 是对手自己的
   * 成手强度（公共牌 + 他的两张牌），是规则层本来就允许使用的公开信息。
...
export function classifyVillainAfterCheck(input: {
  ...
  /*
   * 价值下注是**对手自己的公共牌强度**规则，不读取 Hero 隐藏底牌：
   * 档 0/1 是公共牌面上的强成手带；档 2 仍可能有摊牌价值，但没有足够证据
   * 把它整体当成价值下注。`versusHero` 保留为范围汇总诊断，不参与权重。
   */
  const valueBet = input.tier <= 1;
  /*
