import { writeFileSync } from 'node:fs';
import { join } from 'node:path';

const roundDir = 'D:/德州决策/reports/gtopen-validation/9max-cold-solve-v1';
process.env['ALPHA_GTO_CACHE_DIR'] = join(roundDir, 'cache');
process.env['GTOPEN_URL'] = 'http://127.0.0.1:3747';

const { startAlphaServer } = await import('file:///D:/德州决策/src/app/webServer.ts');

const input = {
  tableSize: 9,
  heroPosition: 'BB',
  heroCards: ['Ah', 'Ad'],
  board: [],
  street: 'PREFLOP',
  effectiveStackBB: 100,
  actionHistory: [
    { position: 'UTG', type: 'RAISE', amountBB: 2.5, street: 'PREFLOP' },
    { position: 'UTG1', type: 'FOLD', street: 'PREFLOP' },
    { position: 'UTG2', type: 'FOLD', street: 'PREFLOP' },
    { position: 'LJ', type: 'FOLD', street: 'PREFLOP' },
    { position: 'HJ', type: 'FOLD', street: 'PREFLOP' },
    { position: 'CO', type: 'FOLD', street: 'PREFLOP' },
    { position: 'BTN', type: 'FOLD', street: 'PREFLOP' },
    { position: 'SB', type: 'FOLD', street: 'PREFLOP' },
  ],
  environment: 'LOW_STAKES_ONLINE',
};

const server = await startAlphaServer({ port: 0, host: '127.0.0.1', gtoBackgroundSolve: false, logPath: null });
try {
  const res = await fetch(server.url + '/api/analyze', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ input, mode: 'OFF' }),
  });
  const body = await res.json();
  const r = body.result ?? body;
  const out = {
    generatedAt: new Date().toISOString(),
    httpStatus: res.status,
    topKeys: Object.keys(r),
    resultKeys: Object.keys(body),
    decisionKeys: Object.keys(r.decision ?? {}),
    diagnosticsKeys: Object.keys(r.diagnostics ?? {}),
    decisionDiagnosticsKeys: Object.keys(r.decision?.diagnostics ?? {}),
    viewModelDebugKeys: Object.keys(r.viewModel?.debug ?? {}),
    raw: r,
    body,
  };
  const outPath = join(roundDir, 'analysis', 'raw-production-response.json');
  writeFileSync(outPath, JSON.stringify(out, null, 2), 'utf8');
  console.log(JSON.stringify({ outPath, topKeys: out.topKeys, decisionKeys: out.decisionKeys, diagnosticsKeys: out.diagnosticsKeys, decisionDiagnosticsKeys: out.decisionDiagnosticsKeys, viewModelDebugKeys: out.viewModelDebugKeys }, null, 2));
} finally {
  await server.close();
}
