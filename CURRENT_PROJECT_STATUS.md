...
> 测试：`test/preflopRaiseDecision.test.ts`（24 项）+ `test/preflopRaiseE2E.test.ts`（12 项）
...
| **Hero 的再加注（5Bet）** | ✅ 已展开：按真实动作顺序回放 Hero 3Bet → Villain 4Bet → Hero 5Bet；比较 FOLD / CALL / 最小合法 5Bet / 全下，逐候选给出响应概率、条件权益、同零点 EV 与支持状态 |
| 翻后加注 / 再加注的有限树 | **未做**（阶段 C） |
| 抽水 | `rakeStatus = 'NOT_APPLIED'` |

**5Bet 收口修正（2026-09-24）**：首次接线时存在一处真实缺陷——动作回放误写成
`Hero CALL → Villain RAISE`，导致 5Bet 候选恒为空；修正为真实的
`Hero RAISE → Villain 4Bet → Hero 5Bet`。同时把候选现金流统一回首次 3Bet 前的决策零点：
Hero 成本包含 3Bet 到 5Bet 的全部新增投入，对手 4Bet 的新增投入进入 5Bet 弃牌收益，
对手 6Bet 后 Hero 弃牌则损失全部被匹配投入。`B-04c2/c3/c4` 分别锁定动作顺序、
独立现金流复算和四类应对的最大 EV 选择；`E2E-02/E2E-10` 锁定理由与诊断区披露。
